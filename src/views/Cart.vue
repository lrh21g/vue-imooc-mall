              ...
              <li v-for="item in cartList" :key="item.productId">
                <div class="cart-tab-1">
                  ...
                  <div class="cart-item-pic">
                    <img v-bind:src="'/images/'+item.productImage" :alt="item.productName">
                  </div>
  ...
  export default {
    data() {
      return {
        cartList: [], // 购物车列表
        delItem: {}, // 用于存储需要删除商品
        modalConfirm: false // 用于控制弹框的显示和隐藏
      }
    },
    computed: {
      // 通过勾选数量和购物车列表数量，判断是否全选
      checkAllFlag() {
    ...
    methods: {
      ...
      // 删除单个商品
      delCartConfirm(item) {
        this.delItem = item // 存储需要删除的单个商品信息
        this.modalConfirm = true // 打开提示框弹窗
      },
      // 关闭提示框弹窗
      closeModal() {
      ...
      delCart() {
        axios.post('/users/cartDel', {
          ...
          let res = response.data
          if (res.status === '0') {
            this.modalConfirm = false // 关闭提示框弹窗
            this.init() // 重新初始化数据
